...
int ispalin(int i,int j){
    ...
    if(ispalind[i][j]!=-1)return ispalind[i][j];

    if(s[i]==s[j])return ispalind[i][j]=ispalin(i+1,j-1);
...
void solve(){
    ...
    int n,q;
    cin>>s;
    int a=ispalin(0,s.sz-1);
    int b=solve(0,s.sz-1);
    // cout<<a<<endl;
    cin>>q;

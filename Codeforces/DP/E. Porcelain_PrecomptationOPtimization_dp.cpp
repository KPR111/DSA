...
int n,m;
int v[105][105],pf[105][105] ,sf[105][105], cache[105][105] ,dp[105][10005];
...
int solve(int i,int ct){
    ...
    for(int j=0;j<=v[i][0];j++){
        ans=max(ans,solve(i+1,ct+j)+cache[i][j]);
    }
    ans=max(ans,solve(i+1,ct));
...
void solve(){
    ...
    fo(i,1,n+1){
        ...
        for(int j=v[i][0];j>=1;j--){
            sf[i][j]=sf[i][j+1]+v[i][j];
        }
    }
    ...
    fo(i,1,n+1){
        ...
        fo(j,0,x+1){
            fo(k,0,j+1){
                int prefvalue=pf[i][k];
                int sufvalue=sf[i][x+1-(j-k)];
                cache[i][j]=max(cache[i][j],prefvalue+sufvalue);
            }
        }
